       ...
       ListView {
        ...
        back: Rectangle {
            ...
            Button {
                ...
                text: "ADD"
                onClicked: {
                    var comp = Qt.createComponent("./GetFile.qml")
                    if (comp.status == Component.Ready) {
                        var fileDialog = comp.createObject(entry, {addfiles: addfiles, desc: adddesc.text, index: editev.index, reader: reader})
                    }
            ...
            Button {
                ...
                text: "OPEN"
                onClicked: {
                        addfiles.getAttachment(addfiles.model1.get(tvattach.currentRow).id)
                }
            }
            TextField {
                id: adddesc
...
/*            TextField {
    ...
    Cont.AttachmentController {
        ...
        onM2readyChanged: {
           //console.log("m2ready")
            if (m2status == 207) {
               ...
               //console.log("getatt", addfiles.model2.get(0).get)
                //Qt.openUrlExternally(addfiles.model2.get(0).get)
                reader.download(addfiles.model2.get(0).get)
        ...
        onReadyChanged: {
                ...
                if (status == 202) {
                   //console.log("attchanged")
                    var ret = JSON.parse(jsn)
                    reader.upload_id = ret["id"]
                    getAllForEvent(editev.index)
                    addfiles.updateAttachment(JSON.stringify({file_size: reader.size}), reader.upload_id)
                }
                if (status ==203) {
    ...
    FileReader {
        ...
        onDownloaded: {
           //console.log("downloaded", reader.size)
            if (Qt.platform.os == "android") {
                //reader.setNotification(addfiles.model1.get(tvattach.currentRow).filename)
                ...
                fileReader.openandroid(addfiles.model1.get(tvattach.currentRow).filename,addfiles.model1.get(tvattach.currentRow).content_type )
        ...
        onUploaded: {
            //console.log("uploaded", reader.size)
